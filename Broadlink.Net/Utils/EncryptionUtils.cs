...
namespace Broadlink.Net
{
	public static class EncryptionExtensions
	{
			...
			using (Aes aesAlg = Aes.Create())
			{
				...
				using (MemoryStream msEncrypt = new MemoryStream())
				{
						...
						= new CryptoStream(msEncrypt, encryptor, CryptoStreamMode.Write))
					{
						...
						//// This padding is an alternative to PaddingMode.Zeros
						//int paddingBytes = GetPaddingBytes(input.Length);
						//if (paddingBytes > 0)
						//	csEncrypt.Write(new byte[paddingBytes], 0, paddingBytes);
					}
		...
		//                byte[] decryptedBytes = csDecrypt.Read(decrypted, 0, input.Length);
		...
		/// <summary>
		/// An alternative to PaddingMode.Zeros.
		/// </summary>
		/// <param name="length"></param>
		/// <param name="multiple"></param>
		/// <returns></returns>
		private static int GetPaddingBytes(int length, int multiple = 16)
		{
			int mod = length % multiple;
			if (mod == 0)
				return 0;
			else
				return multiple - mod;
